...
namespace ModernSchool.DB
{
    public class DataManager
    {
		...
		public async Task<List<IndexesDataStatusViewModel>> IndexesStatus(int school_id, int year)
        {
            return await db.IndexesDataStatuses.FromSqlRaw(@"
				...
				DB as (
					select 
						...
						from Rates r
						where r.IndexId = i.Id and r.SchoolId = " + school_id + @" and r.Year = " + year + @"),0) +

						isnull((select top (1) 1
						from Rates r
						where r.IndexId = i2.Id and r.SchoolId = " + school_id + @" and r.Year = " + year + @"),0) +

						isnull((select top (1) 1
						from Rates r
						where r.IndexId = i3.Id and r.SchoolId = " + school_id + @" and r.Year = " + year + @"),0) +

						isnull((select top (1) 1
						from Rates r
						where r.IndexId = i4.Id and r.SchoolId = " + school_id + @" and r.Year = " + year + @"),0) +

						isnull((select top (1) 1
						from Rates r
						where r.IndexId = i5.Id and r.SchoolId = " + school_id + @" and r.Year = " + year + @"),0) as Selected
		...
		public async Task<double> MaxBallInRepublicOlympiads(int year)
		{
			...
			var data = await db.MaxBallViewModel.FromSqlRaw(@"
				...
				db as (
					select *
					from Rates r
					where r.IndexId in (150,151,152) and r.Year = " + year + @"
				),
				db2 as (
					select *
					from Rates r
					where r.CriteriaId = 106 and r.Year = " + year + @"
				),
				db3 as (
					select 
						distinct s.Id,
		...
		public async Task<double> MaxBallInInternationalOlympiads(int year)
		{
			...
			var data = await db.MaxBallViewModel.FromSqlRaw(@"
				...
				db as (
					select *
					from Rates r
					where r.IndexId = 85 and r.Year = " + year + @"
				),
				db3 as (
					select 
						distinct s.Id,
		...
		public async Task<double> MaxBallInAbitur(int year)
		{
			...
			var data = await db.MaxBallViewModel.FromSqlRaw(@"
				...
				db as (
					select *
					from Rates r
					where r.IndexId = 86 and r.Year = " + year + @"
				),
				db3 as (
					select 
						distinct s.Id,
		...
		public async Task<double> MaxBallInBandlik(int year)
		{
			...
			var data = await db.MaxBallViewModel.FromSqlRaw(@"
				...
				db as (
					select *
					from Rates r
					where r.IndexId = 87 and r.Year = " + year + @"
				),
				db3 as (
					select 
						distinct s.Id,
		...
		public async Task<double> MaxBallInRespublikaTanlov(int year)
		{
			...
			var data = await db.MaxBallViewModel.FromSqlRaw(@"
				...
				db as (
					select *
					from Rates r
					where r.IndexId = 88 and r.Year = " + year + @"
				),
				db3 as (
					select 
						distinct s.Id,
		...
		public async Task<double> MaxBallInXalqaroTanlov(int year)
		{
			...
			var data = await db.MaxBallViewModel.FromSqlRaw(@"
				...
				db as (
					select *
					from Rates r
					where r.IndexId = 89 and r.Year = " + year + @"
				),
				db3 as (
					select 
						distinct s.Id,
